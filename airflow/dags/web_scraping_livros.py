...
with DAG('scraping_books', start_date=datetime(2023, 4, 9),
    ...
    captura_links = PythonOperator(
        ...
        python_callable=exec(open('/home/lucas/projetos/books/captura_links.py').read())
    )
    ...
    extrai_dados = PythonOperator(
        ...
        python_callable=exec(open('/home/lucas/projetos/books/extrai_dados.py').read())
    )

    # Define a terceira tarefa, que será responsável por inserir os dados dos livros no banco de dados.
    insere_bd = PythonOperator(
        task_id='insere_bd',
        python_callable=exec(open('/home/lucas/projetos/books/insere_bd.py').read())
    )
